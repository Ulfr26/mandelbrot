...
fn update_iterations(rl: &RaylibHandle, iterations: &mut i32) {
    let change =
        rl.is_key_down(KeyboardKey::KEY_UP) as i32 - rl.is_key_down(KeyboardKey::KEY_DOWN) as i32;
...
fn main() {
    ...
    let pallette: Vec<Color> = (0..256)
        .map(|x| Color::color_from_hsv(HUE, 1., x as f32 / 255.0))
        .collect();
